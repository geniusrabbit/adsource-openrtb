...
func TestRequestToRTBv2_WithCustomCurrency(t *testing.T) {
	req := makeBidRequest()
	result, _ := requestToRTBv2(req, withCurrencies("EUR", "GBP"))
	assert.Equal(t, []string{"EUR", "GBP"}, result.Cur)
}

func TestRequestToRTBv2_AuctionType(t *testing.T) {
	req := makeBidRequest()
	result, _ := requestToRTBv2(req, requestoptions.WithAuctionType(types.AuctionType(2)))
	assert.Equal(t, 2, result.AuctionType)
}

func TestRequestToRTBv2_TMax(t *testing.T) {
	req := makeBidRequest()
	result, _ := requestToRTBv2(req, requestoptions.WithMaxTimeDuration(200*time.Millisecond))
...
func TestRequestToRTBv2_BidFloor(t *testing.T) {
	...
	imp.BidFloorCPM = 0
	req := makeBidRequestWithImps([]*adtype.Impression{imp})
	result, _ := requestToRTBv2(req, requestoptions.WithBidFloor(0.5))
