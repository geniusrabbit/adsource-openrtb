...
func openrtbV3NativeRequest(req adtype.BidRequester, imp *adtype.Impression, format *types.Format, opts *requestoptions.BidRequestRTBOptions) json.RawMessage {
...
func uopenrtbOpenrtbV3GeoFrom(g *udetect.Geo) *openrtb.Geo {
	return &openrtb.Geo{
		...
		Accuracy:      0,               // Estimated location accuracy in meters
		LastFix:       0,               // Number of seconds since this geolocation fix was established.
		IPService:     0,               // Service or provider used to determine geolocation from IP address
...
func uopenrtbOpenrtbV3DeviceFrom(d *udetect.Device, geo *udetect.Geo) *openrtb.Device {
	...
	return &openrtb.Device{
		UA:           browser.UA,
		Geo:          uopenrtbOpenrtbV3GeoFrom(geo),
		DNT:          int(browser.DNT),
		LMT:          int(browser.LMT),
		IP:           ipV4,
		IPv6:         geo.IPv6String(),
		DeviceType:   uopenrtbOpenrtbV3DeviceType(d.DeviceType),
		Make:         d.Make,
		Model:        d.Model,
		OS:           os.Name,
